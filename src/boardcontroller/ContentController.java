...
//		
		if (id.equals("admin")) {
			RequestDispatcher rd = request.getRequestDispatcher("admin/AdminNoticeContent.jsp"); // 의뢰 = 객체 바인딩
			rd.forward(request, response);
		} else {
			RequestDispatcher rd = request.getRequestDispatcher("user/UserNoticeContent.jsp"); // 의뢰 = 객체 바인딩
			rd.forward(request, response);
		}
	}
