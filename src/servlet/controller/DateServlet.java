...
//		System.out.println("dd = "+dd);
		...
		double LAD = dao.wh_item_LAD(room, yy, mm, dd);
		double LBD = dao.wh_item_LBD(room, yy, mm, dd);
		double LCD = dao.wh_item_LCD(room, yy, mm, dd);
...
//		}
		...
		for (int i = 0; i < mm3.size(); i++) {
			double a = Double.parseDouble(mm3.get(i));
			sum[0] += a;
		}
		for (int i = 0; i < mm2.size(); i++) {
			double a = Double.parseDouble(mm2.get(i));
			sum[1] += a;
		}
		for (int i = 0; i < mm1.size(); i++) {
			double a = Double.parseDouble(mm1.get(i));
			sum[2] += a;
		}
		for (int i = 0; i < mm0.size(); i++) {
			double a = Double.parseDouble(mm0.get(i));
			sum[3] += a;
		}
		for (int i = 0; i < pp1.size(); i++) {
			double a = Double.parseDouble(pp1.get(i));
			sum[4] += a;
		}
		for (int i = 0; i < pp2.size(); i++) {
			double a = Double.parseDouble(pp2.get(i));
			sum[5] += a;
		}
		for (int i = 0; i < pp3.size(); i++) {
			double a = Double.parseDouble(pp3.get(i));
			sum[6] += a;
		}
